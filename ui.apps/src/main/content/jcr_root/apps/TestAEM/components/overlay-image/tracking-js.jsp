...
<script type="text/javascript">
    (function() {
        ...
        var imageEvars = '{ imageLink: "<%= xssAPI.getValidHref(image.get(Image.PN_LINK_URL)) %>", imageAsset: "<%= xssAPI.getValidHref(image.getFileReference()) %>", imageTitle: "<%= xssAPI.encodeForJSString(image.getTitle()) %>" }';
        var tagNodes = imageDiv.getElementsByTagName('A');
        for (var i = 0; i < tagNodes.length; i++) {
            var link = tagNodes.item(i); 
            link.setAttribute('onclick', 'CQ_Analytics.record({event: "imageClick", values: ' + imageEvars + ', collect: false, options: { obj: this }, componentPath: "<%= xssAPI.encodeForJSString(resource.getResourceType()) %>"})');
        }
        <%--
